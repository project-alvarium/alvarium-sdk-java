...
public class SourceCodeAnnotatorTest {
  ...
  public void executeShouldReturnAnnotation()
      throws AnnotatorException, IOException, HashTypeException {
    ...
    String hashAndPath =
        hash.derive(Files.readAllBytes(f1.toPath())) + "  " + f1.toPath().toString() + "\n";
    hashAndPath =
        hashAndPath
            + hash.derive(Files.readAllBytes(f2.toPath()))
            + "  "
            + f2.toPath().toString()
            + "\n";
    ...
    final SourceCodeAnnotatorProps props =
        new SourceCodeAnnotatorProps(
            sourceCodeDir.toPath().toString(), checksumFile.toPath().toString());

    PropertyBag ctx = new ImmutablePropertyBag(Map.of(AnnotationType.SourceCode.name(), props));
